...
model_urls = {
    ...
    'resnet101': 'https://github.com/XingangPan/IBN-Net/releases/download/v1.0/resnet101_ibn_a-59ea0ac6.pth',
...
class ResNet_IBN(nn.Module):
    ...
    def forward(self, x):
        """
        output shape: (b, c, 1, 1), default c = 2048
        """
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def resnet50_ibn_a(last_stride, pretrained=False, input_dim=3, **kwargs):
...
def resnet101_ibn_a(last_stride, pretrained=False, input_dim=3, **kwargs):
