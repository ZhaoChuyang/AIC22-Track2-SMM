...
supported_img_encoders = ["se_resnext50_32x4d", "efficientnet-b2", "efficientnet-b3", "resnet50_ibn_a", "resnet101_ibn_a"]
...
class SiameseLocalandMotionModelBIG(torch.nn.Module):
    def __init__(self, model_cfg):
        ...
        if self.model_cfg.IMG_ENCODER in supported_img_encoders:
            ...
            elif self.model_cfg.IMG_ENCODER == "resnet50_ibn_a":
                self.vis_backbone = resnet50_ibn_a(last_stride=1, pretrained=True)
                self.vis_backbone_bk = resnet50_ibn_a(last_stride=1, pretrained=True)
                self.img_in_dim = 2048
                self.domian_vis_fc = nn.Conv2d(self.img_in_dim, embed_dim, kernel_size=1)
                self.domian_vis_fc_bk = nn.Conv2d(self.img_in_dim, embed_dim, kernel_size=1)
            elif self.model_cfg.IMG_ENCODER == "resnet101_ibn_a":
                self.vis_backbone = resnet101_ibn_a(last_stride=1, pretrained=True)
                self.vis_backbone_bk = resnet101_ibn_a(last_stride=1, pretrained=True)
                self.img_in_dim = 2048
                self.domian_vis_fc = nn.Conv2d(self.img_in_dim, embed_dim, kernel_size=1)
                self.domian_vis_fc_bk = nn.Conv2d(self.img_in_dim, embed_dim, kernel_size=1)
            else:
        ...
        if self.model_cfg.REID_PRETRAIN_WEIGHT:
            state_dict = torch.load(self.model_cfg.REID_PRETRAIN_WEIGHT)
            load_state_dict(state_dict, self.vis_backbone, 'base.')
            load_state_dict(state_dict, self.vis_backbone_bk, 'base.')
        ...
        Camera classifier
        '''
        if self.model_cfg.camera_idloss:
            self.camera_cls = nn.Sequential(
                nn.Linear(embed_dim, embed_dim),
                nn.BatchNorm1d(embed_dim),
                nn.ReLU(),
                nn.Linear(embed_dim, self.model_cfg.NUM_CAMERAS),
            )

        if self.model_cfg.direction_loss:
            self.direction_cls = nn.Sequential(
                nn.Linear(embed_dim, embed_dim),
                nn.BatchNorm1d(embed_dim),
                nn.ReLU(),
        ...
        if self.model_cfg.location_loss:
            self.location_cls = nn.Sequential(
                nn.Linear(embed_dim, embed_dim),
                nn.BatchNorm1d(embed_dim),
                nn.ReLU(),
    ...
    def forward(self, nl_input_ids, nl_attention_mask, crops, motion):
        ...
        if self.model_cfg.camera_idloss:
            camera_preds.append(self.camera_cls(motion_embeds))
        if self.model_cfg.direction_loss:
            direction_preds.append(self.direction_cls(motion_embeds))
        if self.model_cfg.location_loss:
            location_preds.append(self.location_cls(motion_embeds))
        ...
        loss_dict = {
            "pairs": [(visual_car_embeds, lang_car_embeds), (visual_mo_embeds, lang_mo_embeds), (visual_merge_embeds, lang_merge_embeds)],
            "logit_scale": self.logit_scale,
            "cls_logits": cls_logits_results,
            "cam_preds": camera_preds,
...
class TwoBranchModel(torch.nn.Module):
    def __init__(self, model_cfg):
        ...
        if model_cfg.BERT_NAME == 'roberta-large':
            lang_dim = 1024
            if model_cfg.BERT_PRETRAIN_WEIGHT:
                self.bert_model = RobertaModel.from_pretrained(model_cfg.BERT_NAME)
                # state_dict = torch.load(model_cfg.BERT_PRETRAIN_WEIGHT)
                # load_state_dict(state_dict, self.bert_model, 'bert.')

            else:
                self.bert_model = RobertaModel.from_pretrained(model_cfg.BERT_NAME)
        elif model_cfg.BERT_NAME == 'bert-base-uncased':
            lang_dim = 768
            self.bert_model = BertModel.from_pretrained(model_cfg.BERT_NAME)
        else:
            raise NotImplementedError(f'Language Model: {model_cfg.BERT_NAME} not implemented.')
        for p in self.bert_model.parameters():
            p.requires_grad = False
        self.logit_scale = nn.Parameter(torch.ones(()), requires_grad=True)
        ...
        self.car_lang_fc = nn.Sequential(nn.LayerNorm(lang_dim), nn.Linear(lang_dim, embed_dim), nn.ReLU(),
                                            nn.Linear(embed_dim, embed_dim))
        self.motion_lang_fc = nn.Sequential(nn.LayerNorm(lang_dim), nn.Linear(lang_dim, embed_dim), nn.ReLU(),
                                            nn.Linear(embed_dim, embed_dim))
    ...
    def vis_params(self):
        models = [self.vis_backbone, self.vis_backbone_bk, self.domian_vis_fc, self.domian_vis_fc_bk]
        if self.model_cfg.camera_idloss:
            models += [self.camera_cls]
        for model in models:
            for param in model.parameters():
                yield param
        yield self.logit_scale

    def lang_params(self):
        models = [self.bert_model, self.domian_lang_fc]
        for model in models:
            for param in model.parameters():
                yield param

    def get_car_lang_embed(self, car_nl_input_ids, car_nl_attention_masks):
        outputs = self.bert_model(car_nl_input_ids, attention_mask=car_nl_attention_masks)
        car_embeds = torch.mean(outputs.last_hidden_state, dim=1)
        car_embeds = self.car_lang_fc(car_embeds)
        return car_embeds

    def get_car_vis_embed(self, crops):
        visual_embeds = self.domian_vis_fc(self.vis_backbone(crops))
        visual_embeds = visual_embeds.view(visual_embeds.size(0), -1)
        return visual_embeds

    def encode_text(self, nl_input_ids, nl_attention_mask, car_nl_input_ids, car_nl_attention_masks):
        outputs = self.bert_model(nl_input_ids, attention_mask=nl_attention_mask)
        motion_embeds = torch.mean(outputs.last_hidden_state, dim=1)
        motion_embeds = self.motion_lang_fc(motion_embeds)

        outputs = self.bert_model(car_nl_input_ids, attention_mask=car_nl_attention_masks)
        car_embeds = torch.mean(outputs.last_hidden_state, dim=1)
        car_embeds = self.car_lang_fc(car_embeds)

        lang_embeds = torch.cat([car_embeds, motion_embeds], dim=1)
        lang_embeds = F.normalize(lang_embeds, p=2, dim=-1)
        return lang_embeds

    def encode_images(self, crops, motion):
        car_embeds = self.domian_vis_fc(self.vis_backbone(crops))
        car_embeds = car_embeds.view(car_embeds.size(0), -1)

        motion_embeds = self.domian_vis_fc_bk(self.vis_backbone_bk(motion))
        motion_embeds = motion_embeds.view(motion_embeds.size(0), -1)

        visual_embeds = torch.cat([car_embeds, motion_embeds], dim=1)
        visual_embeds = F.normalize(visual_embeds, p=2, dim=-1)
        return visual_embeds

    def forward(self, nl_input_ids, nl_attention_mask, car_nl_input_ids, car_nl_attention_masks, crops, motion):
        lang_motion_embeds = self.bert_model(nl_input_ids, attention_mask=nl_attention_mask)
        lang_motion_embeds = torch.mean(lang_motion_embeds.last_hidden_state, dim=1)
        lang_motion_embeds = self.motion_lang_fc(lang_motion_embeds)

        lang_car_embeds = self.bert_model(car_nl_input_ids, attention_mask=car_nl_attention_masks)
        lang_car_embeds = torch.mean(lang_car_embeds.last_hidden_state, dim=1)
        lang_car_embeds = self.car_lang_fc(lang_car_embeds)
        ...
        cam_preds = []
        if self.model_cfg.camera_idloss:
            cam_preds.append(self.camera_cls(motion_embeds))
            cam_preds.append(self.camera_cls(lang_motion_embeds))

        direction_preds = []
        if self.model_cfg.direction_loss:
            direction_preds.append(self.direction_cls(motion_embeds))
            direction_preds.append(self.direction_cls(lang_motion_embeds))

        location_preds = []
        if self.model_cfg.location_loss:
            location_preds.append(self.location_cls(motion_embeds))
            location_preds.append(self.location_cls(lang_motion_embeds))

        lang_motion_embeds, lang_car_embeds, visual_embeds, motion_embeds = map(
            lambda t: F.normalize(t, p=2, dim=-1),
            (lang_motion_embeds, lang_car_embeds, visual_embeds, motion_embeds)
        )

        loss_dict = {
            "pairs": [(visual_embeds, lang_car_embeds), (motion_embeds, lang_motion_embeds)],
            "logit_scale": self.logit_scale,
            "cls_logits": cls_logits_results,
            "cam_preds": cam_preds,
            "direction_preds": direction_preds,
